...
def generate_element_control_rules(head_nf, tail_nf, chain_id, p4info_helper):
    ...
    curStage = 0
    while cur_nf is not None:
        nextStage = 255 if cur_nf.next_nf is None else (curStage + 1) % 5
        table_entry = p4info_helper.buildTableEntry(
...
if __name__ == '__main__':
    ...
    generate_server_pkt_distribution_rules(
        sfc.chain_head, sfc.id, p4info_helper)
    ...
    rules = []
    rules.extend(generate_element_control_rules(
        sfc.pre_host_chain_head, sfc.pre_host_chain_tail, sfc.id, p4info_helper))
    rules.extend(generate_element_control_rules(
        sfc.post_host_chain_head, sfc.post_host_chain_tail, sfc.id, p4info_helper))

    rules.extend(generate_stage_control_rules(
        sfc.pre_host_chain_head, sfc.pre_host_chain_tail, sfc.id, p4info_helper))
    rules.extend(generate_stage_control_rules(
        sfc.post_host_chain_head, sfc.post_host_chain_tail, sfc.id, p4info_helper))
