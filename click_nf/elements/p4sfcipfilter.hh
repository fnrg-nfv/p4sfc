...
P4SFCIPFilter([CACHING,] ACTION_1 PATTERN_1, ..., ACTION_N PATTERN_N)
...
Filters IP packets. P4SFCIPFilter can have an arbitrary number of filters, which
...
ACTION-PATTERN described above:
  P4SFCIPFilter(file firewall.rules);


The P4SFCIPFilter element has an arbitrary number of outputs. Input packets must
...
Every P4SFCIPFilter element has an equivalent corresponding IPClassifier element
and vice versa. Use the element whose syntax is more convenient for your
needs.

=e

This large P4SFCIPFilter implements the incoming packet filtering rules for the
...
=h program read-only
Returns a human-readable definition of the program the P4SFCIPFilter element
...
If CACHING is enabled, the P4SFCIPFilter element stores the last rule in a cache.
...
{
public:
    P4SFCIPFilter() CLICK_COLD;
    ~P4SFCIPFilter() CLICK_COLD;

    static void static_initialize();
    static void static_cleanup();

    const char *class_name() const override { return "P4SFCIPFilter"; }
...
protected:
    // In caching mode, a set of per P4SFCIPFilter element variables stores caching info
    struct IPFilterCache
    {
        ...
        IPFilterCache(
            ...
            uint64_t cache_hits_nb, uint64_t cache_misses_nb) : last_flow_id(last_flow_id), last_port(last_port),
...
inline bool
P4SFCIPFilter::Primitive::has_transp_proto() const
{
    return _transp_proto >= 0;
}

inline bool
P4SFCIPFilter::Primitive::negation_is_simple() const
{
    if (_type == TYPE_PROTO)
        return true;
...
inline int
P4SFCIPFilter::match(const IPFilterProgram &zprog, const Packet *p)
{
