...
void query_counter_thread(SimpleRouterMgr *simple_router_mgr) {
  printf("Start query thread!\n");
  std::string counter_name("MyIngress.myCounter");
  uint64_t packets;
  uint64_t bytes;
  simple_router_mgr->query_counter(counter_name, 0, &packets, &bytes);
  printf("Counter %s: %lu packets(%lu bytes)\n", counter_name.c_str(), packets, bytes);
}
...
int main(int argc, char *argv[]) {
  ...
  (void) rc;
  assert(rc == 0);
  std::thread thread_obj(query_counter_thread, &simple_router_mgr);
