...
struct SwitchHandler {
  SwitchHandler(GeneralSwitch *general_switch)
      : general_switch(general_switch) { }

  GeneralSwitch *general_switch;
};

struct PacketHandler : public SwitchHandler {
  PacketHandler(GeneralSwitch *general_switch, GeneralSwitch::Packet &&pkt_copy)
      : SwitchHandler(general_switch), pkt_copy(std::move(pkt_copy)) { }
  ...
  void operator()() {
    ...
    if (cpu_hdr.reason == NO_ARP_ENTRY) {
      ...
      general_switch->handle_ip(std::move(pkt_copy), ip_hdr.dst_addr);
      ...
      general_switch->handle_arp(arp_header);
    }
  }

  GeneralSwitch::Packet pkt_copy;
};

struct CounterQueryHandler : public SwitchHandler {
  CounterQueryHandler(GeneralSwitch *general_switch,
                      const std::string &counter_name,
                      size_t index,
                      std::promise<p4v1::CounterData> &promise)
      : SwitchHandler(general_switch), counter_name(counter_name), index(index),
        promise(promise) { }

  void operator()() {
    p4v1::CounterData d;
    int rc = general_switch->query_counter_(counter_name, index, &d);
    if (rc) {
      d.set_packet_count(
...
struct ConfigUpdateHandler : public SwitchHandler {
  ConfigUpdateHandler(GeneralSwitch *general_switch,
                      const std::string &config_buffer,
                      const std::string *p4info_buffer,
                      std::promise<int> &promise)
      : SwitchHandler(general_switch), config_buffer(config_buffer),
        p4info_buffer(p4info_buffer), promise(promise) { }

  void operator()() {
    int rc = general_switch->update_config_(config_buffer, p4info_buffer);
...
class StreamChannelSyncClient {
 public:
  StreamChannelSyncClient(GeneralSwitch *general_switch,
  ...
  void recv_packet_in() {
    recv_thread = std::thread([this]() {
        ...
        while (stream->Read(&packet_in)) {
          ...
          GeneralSwitch::Packet pkt_copy(packet.payload().begin(),
                                           packet.payload().end());
          general_switch->post_event(
              PacketHandler(general_switch, std::move(pkt_copy)));
        }
    });
  }
 ...
 private:
  GeneralSwitch *general_switch{nullptr};
...
GeneralSwitch::GeneralSwitch(int dev_id,
...
GeneralSwitch::~GeneralSwitch() = default;

int
GeneralSwitch::assign(const std::string &config_buffer,
                        const std::string *p4info_buffer) {
...
int
GeneralSwitch::add_one_entry(p4v1::TableEntry *match_action_entry) {
...
int
GeneralSwitch::add_route_(uint32_t prefix, int pLen, uint32_t nhop,
...
int
GeneralSwitch::add_route(uint32_t prefix, int pLen, uint32_t nhop,
                           uint16_t port) {
  int rc = 0;
...
int
GeneralSwitch::add_arp_entry(uint32_t addr,
...
int
GeneralSwitch::assign_mac_addr(uint16_t port,
                                 const unsigned char (&mac_addr)[6]) {
...
int
GeneralSwitch::set_one_default_entry(pi_p4_id_t t_id,
                                       p4v1::Action *action) {
  p4v1::TableEntry match_action_entry;
  match_action_entry.set_table_id(t_id);
  auto entry = match_action_entry.mutable_action();
...
int
GeneralSwitch::set_default_entries() {
  int rc = 0;
...
int
GeneralSwitch::static_config_(UpdateMode update_mode) {
...
void
GeneralSwitch::send_packetout(const char *data, size_t size) {
  packet_io_client->send_packet_out(std::string(data, size));
}

void
GeneralSwitch::handle_arp_request(const arp_header_t &arp_header) {
...
void
GeneralSwitch::handle_arp_reply(const arp_header_t &arp_header) {
...
void
GeneralSwitch::handle_arp(const arp_header_t &arp_header) {
...
void
GeneralSwitch::send_arp_request(uint16_t port, uint32_t dst_addr) {
  for (const auto &iface : ifaces) {
...
void
GeneralSwitch::handle_ip(Packet &&pkt_copy, uint32_t dst_addr) {
...
void
GeneralSwitch::add_iface_(uint16_t port_num, uint32_t ip_addr,
                            const unsigned char (&mac_addr)[6],
...
void
GeneralSwitch::add_iface(uint16_t port_num, uint32_t ip_addr,
                           const unsigned char (&mac_addr)[6]) {
...
int
GeneralSwitch::query_counter(const std::string &counter_name, size_t index,
...
int
GeneralSwitch::query_counter_(const std::string &counter_name, size_t index,
...
int
GeneralSwitch::update_config(const std::string &config_buffer,
                               const std::string *p4info_buffer) {
  std::promise<int> promise;
  auto future = promise.get_future();
...
int
GeneralSwitch::update_config_(const std::string &config_buffer,
                                const std::string *p4info_buffer) {
