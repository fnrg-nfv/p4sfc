...
general_switch_t GS_connect(const char *grpc_addr, const char *config_path,
                            const char *p4info_path, int dev_id);

void GS_run(general_switch_t self);

void GS_query_counter(general_switch_t self, char *counter_name, int index,
                      uint64_t *packets, uint64_t *bytes);

int GS_add_table_entry(general_switch_t untyped_self,
